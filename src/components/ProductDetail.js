...
const ProductDetail = ({ product, isOpen, onClose }) => {
  ...
  // Direct initialization - component remounts on key change so no need for useEffect
      ...
      <div className="fixed inset-x-0 bottom-0 top-[70px] sm:top-[80px] lg:top-[90px] z-50 flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
        <div className="bg-white rounded-lg shadow-2xl w-full max-w-md sm:max-w-2xl lg:max-w-5xl xl:max-w-6xl my-auto max-h-[calc(100vh-80px)] sm:max-h-[calc(100vh-100px)] overflow-hidden relative border-2 sm:border-4 border-[#E7BD8B]">
          ...
          <div className="absolute top-2 sm:top-4 right-2 sm:right-4 flex gap-1 sm:gap-2 z-10">
            <button className="bg-[#234433] text-white w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center hover:bg-[#234433]/90 transition-colors shadow-lg">
            ...
            <button
              onClick={onClose}
              className="bg-[#234433] text-white w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center hover:bg-[#234433]/90 transition-colors shadow-lg"
            >
